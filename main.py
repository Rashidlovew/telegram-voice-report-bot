...
def extract_fields(text):
    ...
    return {
        ...
        "TechincalOpinion": lines[7].replace("الرأي_الفني", "").strip(),
    }
...
def handle_voice(update, context):
    try:
        update.message.reply_text("📥 جارٍ تحميل المقطع الصوتي...")
        file = update.message.voice.get_file()
        file.download("voice.ogg")

        update.message.reply_text("🎙️ جارٍ تحويل الصوت إلى نص...")
        text = transcribe("voice.ogg")
        update.message.reply_text(f"📋 تم تحويل الصوت إلى نص:\n{text}")

        update.message.reply_text("📄 جارٍ إنشاء التقرير...")
        data = extract_fields(text)
        generate_report(data)

        update.message.reply_text("📬 جارٍ إرسال التقرير إلى بريدك الإلكتروني...")
        send_email()
